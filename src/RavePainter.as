package {
    ...
    /**
     * Class aimed to make polygonal shapes written in spark path notation
     *
     */

    public class RavePainter extends Sprite {

        private static const invertTransform:ColorTransform = new ColorTransform(-1, -1, -1, 1, 255, 255, 255, 0);
        ...
        public function RavePainter(width:int, height:int) {
            super();
        ...
        private function recognizeGraphics():void {
            var str:String = "";
            for each (var head:DirectedVertex in pathHeads) {
                var x:int = head.x + 1;
                var y:int = head.y;
                str += "M " + x + " " + y + " ";
                var further:DirectedVertex = head.next;
                while (further) {
                    str += "L " + (further.x + 1) + " " + further.y + " ";
                    further = findFurtherVertexOnSegment(further);
                }
            }
            str += "Z";
            dispatchEvent(new RecognizeCompleteEvent(str));
            pathHeads.length = 0;
        }
...
class DirectedVertex {
    ...
    public function followTo(direction:int):DirectedVertex {
        switch (direction) {
            case RavePainter.DIR_RIGHT:
                return toRight();
            case RavePainter.DIR_LEFT:
                return toLeft();
            case  RavePainter.DIR_TOP:
                return toTop();
            case RavePainter.DIR_DOWN:
                return toBottom();
        }
        return null;
    }
    ...
    public function toRight():DirectedVertex {
        ...
        directedVertex._direction = RavePainter.DIR_RIGHT;
        next = directedVertex;
    ...
    public function toLeft():DirectedVertex {
        var directedVertex:DirectedVertex = new DirectedVertex(x - 1, y);
        directedVertex._direction = RavePainter.DIR_LEFT;
        next = directedVertex;
        return directedVertex;
    }
    ...
    public function toTop():DirectedVertex {
        ...
        directedVertex._direction = RavePainter.DIR_TOP;
        next = directedVertex;
        return directedVertex;
    }
    ...
    public function toBottom():DirectedVertex {
        var directedVertex:DirectedVertex = new DirectedVertex(x, y + 1);
        directedVertex._direction = RavePainter.DIR_DOWN;
        next = directedVertex;
        return directedVertex;
    }
